...
public class PanelGrafo extends javax.swing.JPanel {
	...
	public static boolean getNodos(Nodo n) {
		...
		if (nodos.size() > 0) {
			for (int i = 0; i <= nodos.size(); i++) {
				...
				System.out.println("PanelGrafo.getNodos nodo= " + nombre);
					...
					try {
						if (!nodo2) {
							...
							if (destino != null) {
								...
								int p = 1;
								/*
								 * while (p==0) { try { peso= JOptionPane.showInputDialog("Ingrese el peso >0");
								 * p=Integer.parseInt(peso); } catch (Exception ex) { } }
								 */
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < arcos.size(); i++) {
			System.out.println("arcos = " + arcos.get(i).getDestino().getNombre());
			...
			/*
			 * g.setColor(Color.WHITE); g.drawString(String.valueOf(arcos.get(i).getPeso()),
			...
			/*
			 * (la coordenadas de la ventana es al revez, calculo de la variacion de "x" y
	...
	// PARA OBTENER EL NODO SELECCIONADO MEDIANTE LAS CORDENADAD XY DE MOUSE
	// LISTENER
	public Nodo getNodoPorXY(int x, int y) {
		for (int i = 0; i < nodos.size(); i++) {
			System.out.println("PanelGrafo.getNodoPorXY");
			if ((x >= nodos.get(i).getPunto().x && x <= nodos.get(i).getPunto().x + 50)
					&& (y >= nodos.get(i).getPunto().y && y <= nodos.get(i).getPunto().y + 50))
				return nodos.get(i);
		}
		return null;
	}
